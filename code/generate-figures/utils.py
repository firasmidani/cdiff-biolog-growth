...
rcParams['savefig.pad_inches'] = 0.05 # ~1 cm
...
def tidy_labels(ticklabels):
    '''
    Converts substrate names to tidier labels to be used in plots. 
    
    Args:
        ticklabels (list of text instances), e.g. generated by matplotlib with ax.get_yticklabels()
            all ticklabels must have corresponding row in susbtrate_annotations.tsv
    ...
    df_sub_mapping = df_sub_mapping[df_sub_mapping.Figure=='x']
    dict_labels = df_sub_mapping.loc[:,'Substrate_Label'].to_dict()
    
    new_labels = [dict_labels[ii.get_text()] for ii in ticklabels]
    
    return new_labels

def tidy_label(substrate):
    '''
    Converts substrate name to tidier label.
    
    Args:
        substrate (str), must have corresponding row in susbtrate_annotations.tsv
    ...
    df_sub_mapping = read_csv(f"{dir_config}/substrate_annotations.tsv")
    label = df_sub_mapping.loc[substrate,'Substrate_Label']
    return label